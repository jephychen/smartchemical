/**
 * 
 */
package com.smartchemical.thirdparty.payapi.mobaopay;

/**
 * @author Jephy
 *
 */
public class Mobo360Config {
	// 请选择签名类型， MD5、CER(证书文件)、RSA
		public static final String SIGN_TYPE = "MD5";
//		public static final String SIGN_TYPE = "CER";
//		public static final String SIGN_TYPE = "RSA";

		//******************配置商户基本信息
		// 商户ID
		public static String PLATFORM_ID = "210001510009249";
		// 商户帐号
		public static String MERCHANT_ACC = "210001510009249";
		//******************配置商户基本信息***结束
		
		//******************使用MD5签名方式需配置下列参数
		// MD5签名的key值
		public static String MD5_KEY = "4808b8952925161d45276b308ebe0924";
		//******************MD5签名方式需配置***结束
		
		//******************使用证书签名方式需配置下列参数
		// 商户私钥文件--用于商户数据签名
		public static String PFX_FILE = "c:/temp/test/merchtest.pfx";
		// 摩宝支付公钥文件--用于摩宝支付返回数据验签
		public static String CERT_FILE = "c:/temp/test/mobaopay.cer";
		// 私钥文件密码
		public static String PASSWD = "merchtest";
		//******************证书签名方式需配置***结束

		//******************使用RSA签名方式需配置下列参数
		// 用于签名的商户RSA私钥
		public static String RSA_MERCH_PRIVATE_KEY = "";
		// 用于验签的摩宝支付RSA公钥
		public static String RSA_MBP_PUBLIC_KEY = "";
		//******************RSA签名方式需配置***结束

		// 通知地址
		public static String MERCHANT_NOTIFY_URL = "http://www.zhixuanhuaxue.com/smartchemical-web/OnlinePayCallback.action";
		//public static String MERCHANT_NOTIFY_URL = "http://127.0.0.1:8080/MobaoPayExample/callBack.jsp";
		// 摩宝支付网关
		// 测试地址
		//public static String MOBAOPAY_GETWAY = "https://182.148.123.7/cgi-bin/netpayment/pay_gate.cgi";
		// 正式地址
		public static String MOBAOPAY_GETWAY = "https://trade.mobaopay.com/cgi-bin/netpayment/pay_gate.cgi";
		
		// 摩宝支付接口版本
		public static final String MOBAOPAY_API_VERSION = "1.0.0.0";

		// 接口名称
		public static final String MOBAOPAY_APINAME_PAY = "WEB_PAY_B2C";
		public static final String MOBAOPAY_APINAME_QUERY = "MOBO_TRAN_QUERY";
		public static final String MOBAOPAY_APINAME_REFUND = "MOBO_TRAN_RETURN";
}

...
public class QueryOrderAction implements Action {
	...
	private String pageActionName = "MerchantQueryOrder";
	...
	private void setAllOrderPaging() throws Exception {
		...
		int totalRecordCount = orderDao.getPOsCountByMerchantCompanyStatus(user.getCompany().getCompanyId(), status, ScUtil.makeSinceDate(lastMonth));
		setPaginator(totalRecordCount);
		orders = orderDao.getPOsByMerchantCompanyStatus(user.getCompany().getCompanyId(), status, PAGE_SIZE, 
	...
	private void setWaitingReceivedPaging() throws Exception {
		...
		int totalRecordCount = orderDao.getPOsCountByMerchantCompanyStatusDeliveryType(user.getCompany().getCompanyId(), status, deliveryType, ScUtil.makeSinceDate(lastMonth));
		setPaginator(totalRecordCount);
		orders = orderDao.getPOsByMerchantCompanyStatusDeliveryType(user.getCompany().getCompanyId(), status, deliveryType, PAGE_SIZE, 
				paginator.getCurrentPage(), ScUtil.makeSinceDate(lastMonth));
	}

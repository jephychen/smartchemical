...
function saveProduct(queryType, productId){
	...
	checkProductPrice();
	checkStockLevel();
	if (!isProductNameValid || !isProductNoValid || !isProductDescValid || !isProductPriceValid || !isStockLevelValid){
		return;
	}
	...
	var merchantCompanyId = document.getElementById("pop_content_merchantcompany_dropdown").value;
	...
	var merchantCompanyInput = document.createElement("input");
	merchantCompanyInput.setAttribute("name", "merchantCompanyId");
	merchantCompanyInput.setAttribute("value", merchantCompanyId);
	merchantCompanyInput.setAttribute("type", "hidden");
	submitForm.appendChild(merchantCompanyInput);
	
	submitForm.method = "post";
	submitForm.action = "AdminProductSubmit_editSave.action";
	submitForm.submit();
}
